...
static void period_log_stat(bool flag)
{
	...
	INFO_LOG(
			"Period(%usecs): %u requests sent.", g_options.ratio, g_ratio_sent_req);
	INFO_LOG(
			"Period(%usecs): %u responses recv.", g_options.ratio, g_ratio_recved_res);
	g_ratio_sent_req = 0;
...
static Buffer& get_request_data()
{
	...
	} else
	{
		...
		while (it != g_options.vars.end())
		{
			...
			{
				string_replace(newstr, key, value);
			}
			it++;
		}
...
static void stop_serv()
{
	...
	if (NULL != g_serv)
	{
		...
		g_serv = NULL;
		INFO_LOG(
				"Finish: %u requests sent, %u responses recv.", g_sent_count, g_recved_res);
		INFO_LOG(
				"Cost %llums to execute benchmark test.", (now - g_start_time));
...
HttpClientHandler::HttpClientHandler() :
		m_request_sent(false), m_connected(false), m_response_recved(false), m_client(
				NULL)
{
}
...
int start_benchmark(HttpBenchmarkOptions& options, Buffer& requestData)
{
	...
	g_serv->GetTimer().ScheduleHeapTask(
			make_fun_runnable(period_log_stat, false), options.ratio * 1000,
			options.ratio * 1000);
	g_serv->Start();
	return 0;
}
