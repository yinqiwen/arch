...
bool SocketChannel::DoConfigure(const ChannelOptions& options)
{
	...
	int flag = 1;
	if (options.tcp_nodelay && (setsockopt(m_fd, SOL_SOCKET, TCP_NODELAY,
	        &flag, sizeof(flag)) < 0))
	{
		WARN_LOG("init_sock_opt: could not disable Nagle: %s",
				strerror(errno));
	}
...
bool SocketChannel::DoBind(Address* local)
{
	...
	if (addr.IsUnix())
	{
		struct sockaddr_un* pun =
		        (struct sockaddr_un*) &(addr.GetRawSockAddr());
...
//		}
		...
		if (aeCreateFileEvent(m_service.GetRawEventLoop(), fd, AE_READABLE
		        | AE_WRITABLE, Channel::IOEventCallback, this) == AE_ERR)
		{
			::close(fd);
			return -1;
		}
