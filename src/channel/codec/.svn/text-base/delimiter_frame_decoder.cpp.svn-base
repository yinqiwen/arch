...
DelimiterBasedFrameDecoder::DelimiterBasedFrameDecoder(uint32 maxFrameLength,
    ...
    m_delimiters(delimiters), m_maxFrameLength(maxFrameLength),
            m_stripDelimiter(stripDelimiter), m_discardingTooLongFrame(false)
{
    if (m_delimiters.empty())
    {
        throw Exception("Delimiters is empty!");
    }
}

bool DelimiterBasedFrameDecoder::Decode(ChannelHandlerContext& ctx, Channel* channel,
        Buffer& buffer, Buffer& frame)
{
    ...
    if (minDelim != NULL)
    {
        ...
        //Buffer* frame = NULL;
        //NEW(frame, Buffer);
        frame.EnsureWritableBytes(m_stripDelimiter ? minFrameLength
