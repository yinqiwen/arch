...
#include "concurrent/atomic/atomic_common.h"
#include "concurrent/atomic/atomic_native.h"
#include <errno.h>
...
inline static void futex_get(futex_lock_t* lock)
{
	...
	else if (unlikely(v==2))
	{ /* if contended, optimize for the one waiter
	 case */
...
inline static int futex_get_timeout(futex_lock_t* lock, const struct timespec *timeout)
{
	int v;
#ifdef ADAPTIVE_WAIT
	register int i=ADAPTIVE_WAIT_LOOPS;
	...
	{ /* if contended, optimize for the one waiter
	 case */
		...
		do
		{
			int ret = sys_futex(&(lock)->val, FUTEX_WAIT, 2, timeout, 0, 0);
