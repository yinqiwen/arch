...
static ngx_uint_t ngx_pagesize_shift = 12;
...
{
	if (size < sizeof(ngx_slab_pool_t))
	{
		return 0;
	}
	ngx_slab_pool_t* pool = (ngx_slab_pool_t*) addr;
	pool->end = ((u_char*) addr) + size;
	pool->min_shift = 3;
	pool->addr = addr;
	...
	pool->start
	        = (u_char *) ngx_align_ptr((uintptr_t) p + pages * sizeof(ngx_slab_page_t),
			        ngx_pagesize);
...
ngx_slab_alloc(ngx_slab_pool_t *pool, size_t size)
{
	...
	//ngx_spinlock(pool->mutex, ngx_pid, 1024);

	p = ngx_slab_alloc_locked(pool, size);

	ngx_shmtx_unlock(&pool->mutex);
	//ngx_atomic_cmp_set(pool->mutex, ngx_pid, 0);
	...
	if (page->next != page)
	{
		...
		if (shift < ngx_slab_exact_shift)
		{
			...
			do
			{
				...
				map = (1 << (ngx_pagesize_shift - shift)) / (sizeof(uintptr_t)
				        * 8);
				...
				for (n = 0; n < map; n++)
				{
					...
					if (bitmap[n] != NGX_SLAB_BUSY)
					{
						...
						for (m = 1, i = 0; m; m <<= 1, i++)
						{
							...
							i = ((n * sizeof(uintptr_t) * 8) << shift) + (i
							        << shift);
		...
		{ /* shift > ngx_slab_exact_shift */
			...
			do
			{
				if ((page->slab & NGX_SLAB_MAP_MASK) != mask)
				{
					...
					for (m = (uintptr_t) 1 << NGX_SLAB_MAP_SHIFT, i = 0; m
					        & mask; m <<= 1, i++)
					{
						if ((page->slab & m))
						{
...
void ngx_slab_free_locked(ngx_slab_pool_t *pool, void *p)
{
	...
	{
		ngx_slab_error(pool, 0, "ngx_slab_free(): outside of pool");
		goto fail;
	}
...
ngx_slab_alloc_pages(ngx_slab_pool_t *pool, ngx_uint_t pages)
{
	...
	ngx_slab_error(pool, 0, "ngx_slab_alloc() failed: no memory");
