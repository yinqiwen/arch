        ...
        const std::string& passwd)
{
    ...
    if (NULL == mysql_real_connect(m_handler, host.c_str(), user.c_str(),
        ...
        va_list ap)
{
    ...
    struct st_mysql_res * results = m_bRemote ? mysql_use_result(m_handler)
    : mysql_store_result(m_handler);
...
int MySQLConnection::GetErrNum() const
{
    unsigned int ret = 0;
    if (m_handler)
...
int MySQLConnection::Connect(const Properties& props)
{
    ...
    if (!portstr.empty())
    {
        ...
        {
            snprintf(m_szErrMsg, sizeof(m_szErrMsg) - 1,
                    "Invalid 'Port' property specified",
                    mysql_errno(m_handler), m_szSQL, mysql_error(m_handler));
            return -1;
        }
    }
    SetProperties(props);
