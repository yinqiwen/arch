...
int ManagerProcess::CurrentControlProcess(Channel* ch, ServiceIndex& idx)
{
	DelegateChannelControlProcessTable::iterator found = m_delegate_channel_control_table.find(ch->GetReadFD());
	if(found == m_delegate_channel_control_table.end())
	{
		return -1;
	}
	idx = found->second;
