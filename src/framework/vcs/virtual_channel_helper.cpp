...
int VirtualChannelHelper::SocketOpenRequest(bool is_tcp)
{
    if (NULL != g_service_proc)
    {
...
int VirtualChannelHelper::CloseSocket(uint32 socket_channel_id)
{
    if (NULL != g_service_proc)
    {
        Channel* dst_channel = NULL;
...
static int send_channel_control_event(ServiceIndex index, Channel* ch,
        uint32 type)
{
    if (NULL == g_service_proc)
    {
    ...
    event.SetSrcType(g_service_proc->GetServiceType());
    event.SetSrcIndex(g_service_proc->GetProcessIndex());
    event.SetDstType(g_service_proc->GetServiceType());
    event.SetDstIndex(index);
    uint64 info = GENERATE_DELEGATE_CHANNEL_CONTROL_EVENT(type,
            ch->GetReadFD());
    event.SetEventInfo(info);
    event.SetIPCCtrlType(TRANSMIT_CHANNEL_CONTROL);
    if (!g_service_proc->GetIPCChannel()->Write(ev))
    {
        return -1;
    }
    g_service_proc->GetIPCChannel()->Flush();
    return 0;
}

int VirtualChannelHelper::TransferDelegateChannelControlRequest(
        ServiceIndex index, Channel* ch)
{
    DEBUG_LOG(
            "VirtualChannelHelper::TransferDelegateChannelControlRequest with fd:%d", ch->GetReadFD());
    return send_channel_control_event(index, ch,
            DELEGATE_CHANNEL_CONTROL_REQUEST);
}

int VirtualChannelHelper::TransferDelegateChannelControlResponse(
        ServiceIndex index, Channel* ch)
{
    DEBUG_LOG(
            "VirtualChannelHelper::TransferDelegateChannelControlResponse with fd:%d", ch->GetReadFD());
    return send_channel_control_event(index, ch,
            DELEGATE_CHANNEL_CONTROL_RESPONSE);
}

int VirtualChannelHelper::DefaultDelegateSocketAcceptedCallBack(
        ServerSocketChannel* server, ClientSocketChannel* client)
{
    if (server->ConnectedSockets() % 1 == 0)
    {
        ServiceIndex current = g_service_proc->GetProcessIndex();
        ServiceIndex next = current + 1;
        if (next
                >= g_service_proc->GetServiceProcessFactory()->GetProcessCount())
        {
            next = 0;
        }

        TransferDelegateChannelControlRequest(next, server);
        return -1;
    }
    return 0;
}
