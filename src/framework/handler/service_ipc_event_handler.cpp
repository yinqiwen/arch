...
//				}
				...
				{
					uint64 info = event->GetEventInfo();
					uint32 type = RETRIVE_DELEGATE_TYPE(info);
					uint32 fd = RETRIVE_DELEGATE_FD(info);
					Channel* ch = GetServiceProcess()->GetDelegateChannel(fd);
					if (NULL == ch)
					{
						WARN_LOG("No channel found for FD:%u", fd);
						break;
					}
					switch (type)
					{
						case DELEGATE_CHANNEL_CONTROL_REQUEST:
						{
							ch->AttachFD();
							VirtualChannelHelper::TransferDelegateChannelControlResponse(
							        event->GetSrcIndex(), ch);
							break;
						}
						case DELEGATE_CHANNEL_CONTROL_RESPONSE:
						{
							ch->DetachFD();
							break;
						}
						...
						{
							ERROR_LOG(
							        "Unsupport transfer channel control type:%u", type);
						}
					}
					break;
				}
