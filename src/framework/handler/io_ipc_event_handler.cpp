	...
	switch (event->GetSocketCtrlType())
	{
		case SOCKET_OPEN:
		{
			...
			if (IS_TCP_CHANNEL(event->GetSocketChannelID()))
			{
				...
				{
					ch = m_handler->TCPSocketOpenRequest(event->GetAddonInfo());
				}
				else if (NULL != unix_addr)
				{
					ch = m_handler->UnixSocketOpenRequest(unix_addr->GetPath(), event->GetAddonInfo());
				}
			}
			else
			{
				ch = m_handler->UDPSocketOpenRequest(event->GetAddonInfo());
			}
			break;
		}
		case SOCKET_CONNECT:
				...
				if (IS_TCP_CHANNEL(event->GetSocketChannelID()))
				{
					if (NULL != addr)
					{
						ch = m_handler->TCPSocketConnectRequest(addr->GetHost(),
						        addr->GetPort(),event->GetAddonInfo());
					}
					else if (NULL != unix_addr)
					{
						ch = m_handler->UnixSocketConnectRequest(
						        unix_addr->GetPath(),event->GetAddonInfo());
					}
				}
				else
				{
